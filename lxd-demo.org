...
+ Full virtualization.  A guest does not have a kernel!  If you want
  to run Windows, BSD, or something else, then use libvirt and
  virt-manager.
+ Application-specific containers (i.e. Docker, Kubernetes-like, CNCF,
  or alphabet soup compliant)
...
#+begin_src sh
  ...
  # If you prefer to init and launch in a separate step
  lxc init impish i1
  lxc start i1
...
#+begin_src sh
  lxc file pull i1/etc/hosts .
  lxc file push hosts i1/etc/hosts
#+end_src
...
For this demo, we will run Apache and mount a simple website to
=/var/www/html=.

#+begin_src sh
  lxc launch ubuntu:impish apache
  lxc ls
  lxc shell apache
  ...
  lxc config device add apache html disk source=/home/kyle/git/oclug/lxd-demo/site path=/var/www/html
...
#+begin_src sh
  # On host
  id
  lxc config set apache raw.idmap 'both 2290 2290'
  lxc restart apache
...
#+begin_src sh
  # On host
  lxc config show apache
  export EDITOR=vi
  lxc config edit apache
#+end_src
...
#+begin_src sh
  ...
  lxc publish apache --alias apache
  lxc image export apache
...
#+begin_src sh
  ...
  cd /var/www
  mv html html.old
  sudo ln -s /mnt/home/kyle/git/oclug/lxd-demo/site html
...
* Real-world example: Web server on Rocky Linux 8
